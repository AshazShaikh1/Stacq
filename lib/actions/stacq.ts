...
export interface StacqCreateResponse {
  success?: boolean;
  stacqId?: string;
  slug?: string;
  error?: string;
...
export async function createStacq(
  title: string,
  description: string,
  category: string,
  thumbnail?: string,
  ...
  const validation = stacqSchema.safeParse({
    title,
    description,
    category,
    thumbnail,
  ...
  if (!stacqId) {
    return {
      error:
        "Stacq saved, but database delayed returning the ID. Check your feed!",
    };
  }

...
export default async function ProfilePage({
  ...
  const { data: profileUser, error: profileError } = await supabase
    .from("users")
    .select("id, username, display_name, avatar_url, role") // Added 'role'
    .eq("username", username)
    .single();
  ...
  const totalViews =
    viewsResult.data?.reduce((sum, collection) => {
  ...
  if (tab === "card" || tab === "cards") {
    ...
    if (tab === "collection" || tab === "created") {
      // Support both 'collection' and legacy 'created'
      ...
      if (saves && saves.length > 0) {
        ...
        collectionsQuery = collectionsQuery.eq(
          "id",

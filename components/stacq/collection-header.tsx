      ...
      <header className="mb-10 relative bg-surface p-6 md:p-8 rounded-3xl border border-border shadow-sm w-full">
        ...
        <div className="flex flex-wrap items-center gap-3 pt-6 border-t border-border mt-6">
          <button
            onClick={handleSave}
            disabled={loading}
            className="inline-flex items-center justify-center bg-primary hover:bg-primary-dark text-primary-foreground rounded-full px-8 h-12 font-bold shadow-emerald shadow-sm transition-transform hover:scale-105 active:scale-95 disabled:opacity-50 cursor-pointer"
          >
            {loading ? (
          ...
          <button
            ...
            disabled={loading}
            className="inline-flex items-center justify-center rounded-full px-8 h-12 font-bold text-muted-foreground hover:bg-surface-hover transition-colors cursor-pointer"
          >
  ...
  return (
    <header className="mb-10 relative group inline-block w-full transition-all">
      <div className="text-center md:text-left">
        <Badge
          variant="outline"
          className="mb-4 bg-primary/10 text-primary border-primary/20 px-3 py-1 font-extrabold tracking-tight"
        >
        ...
        {stacq.description && (
          <div className="bg-primary/5 py-4 px-6 md:py-6 md:px-8 rounded-2xl md:rounded-3xl border-l-[6px] border-primary inline-block w-full sm:w-auto text-left shadow-sm max-w-full md:max-w-3xl transition-all">
            <p
              className={`text-lg md:text-xl italic text-foreground/80 font-medium leading-relaxed whitespace-pre-wrap ${!isExpanded ? "line-clamp-4" : ""}`}
            >
              &quot;{stacq.description}&quot;
            </p>
            {isLongDescription && (
      ...
      {isOwner && (
        <div className="flex md:absolute md:top-4 md:right-4 md:opacity-0 group-hover:opacity-100 transition-all duration-300 gap-2 z-10 mt-6 md:mt-0 justify-center">
          <button
            ...
            className="p-3 text-muted-foreground hover:text-primary bg-background hover:bg-surface rounded-full shadow-sm hover:shadow-md border border-border hover:border-primary cursor-pointer transition-all active:scale-95"
          >
          ...
          <AlertDialog>
            ...
            <AlertDialogContent className="rounded-2xl max-w-[90vw] sm:max-w-md">
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  Are you sure you want to delete this stacq and all of its
                  resources? This action cannot be undone.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter className="flex-col sm:flex-row gap-2">
                <AlertDialogCancel className="rounded-full w-full sm:w-auto mt-0">
                  Cancel
                </AlertDialogCancel>
                <AlertDialogAction

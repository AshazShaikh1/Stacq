...
export function ResourceCard({
  ...
  const imgSrc = imgError || !thumbnail ? FALLBACK_IMG : thumbnail;
        ...
        <div className="mt-4 pt-4 border-t border-border relative">
          <div className="flex gap-3 items-start">
            ...
            <div className="flex-1">
              <p
                className={`text-sm md:text-base text-foreground/70 leading-relaxed font-medium pr-10 whitespace-pre-wrap ${!isExpanded ? "line-clamp-3" : ""}`}
              >
          ...
          {isOwner && (
            <div className="absolute bottom-0 right-0 flex items-center gap-1 bg-background/80 backdrop-blur-sm pl-2 py-1 rounded-tl-xl transition-opacity opacity-100 md:opacity-0 group-hover:opacity-100">
              <Dialog open={openEdit} onOpenChange={setOpenEdit}>
                <DialogTrigger
                  className="p-2 text-muted-foreground hover:text-primary rounded-lg hover:bg-primary/5 transition-colors cursor-pointer"
                ...
                <DialogContent className="sm:max-w-2xl rounded-3xl p-6 sm:p-8">
                  <DialogHeader>
                    <DialogTitle className="text-2xl font-black tracking-tight mb-4">
                      Edit Resource
                    </DialogTitle>
                  </DialogHeader>
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <Input
                          ...
                          className="h-11 sm:h-12 text-base font-bold bg-background rounded-xl"
                        />
                      </div>
                      <div>
                        <label className="text-[10px] sm:text-xs font-bold text-primary uppercase tracking-widest mb-1.5 block">
                      ...
                      <div className="space-y-2">
                        <label className="text-[10px] sm:text-xs font-bold uppercase text-primary tracking-widest">
                          Section (Optional)
                        </label>
                        {isCreatingSection ? (
                          <div className="flex gap-2 animate-in fade-in slide-in-from-left-2 duration-300">
                            ...
                            <Button
                              ...
                              variant="outline"
                              className="h-12 w-12 rounded-xl text-muted-foreground p-0 hover:bg-destructive/5 hover:text-destructive"
                            >
                              <X className="w-4 h-4" />
                            </Button>
                          </div>
                          ...
                          >
                            ...
                            <SelectContent className="rounded-xl border-border shadow-2xl">
                              ...
                              <SelectItem
                                value="new-section"
                                className="text-primary py-2.5 border-t border-border/50 mt-1 font-bold"
                              >
                                <div className="flex items-center gap-2">
                                  <PlusSquare className="w-4 h-4" />+ Add New
                                  Section
                                </div>
                              </SelectItem>
                            </SelectContent>
                  ...
                  <div className="pt-6 border-t border-border mt-6">
                    <Button
                      onClick={handleSave}
                      disabled={saving}
                      className="w-full bg-primary hover:bg-primary-dark text-primary-foreground rounded-full h-14 font-black shadow-emerald shadow-lg active:scale-95 transition-transform"
                    >
                      {saving ? (
              ...
              >
                <AlertDialogTrigger
                  className="p-2 text-muted-foreground hover:text-destructive rounded-lg hover:bg-destructive/10 transition-colors cursor-pointer outline-none"
